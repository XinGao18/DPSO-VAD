...
class CLIPVAD(nn.Module):
    def __init__(self,
        ...
        self.classifier = nn.Linear(visual_width, 1)  # 二元分类器

        self.clipmodel, _ = clip.load("ViT-B/16", device) # 预训练模型为ViT-B/16
    ...
    def forward(self, visual, padding_mask, text, lengths):
        visual_features = self.encode_video(visual, padding_mask, lengths) # [128, 256, 512]
        logits1 = self.classifier(visual_features + self.mlp2(visual_features)) # [128,256,1]

        text_features_ori = self.encode_textprompt(text) # [14, 512]

        text_features = text_features_ori  
        logits_attn = logits1.permute(0, 2, 1) # [128,1,256] -> [B, C, T]
        visual_attn = logits_attn @ visual_features # [128, 1, 512]
        ...
        text_features = text_features + self.mlp1(text_features) # [128, 14, 512]

        visual_features_norm = visual_features / visual_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features / text_features.norm(dim=-1, keepdim=True)
        text_features_norm = text_features_norm.permute(0, 2, 1) [128, 512, 14]
        # [128, 256, 14] [128, 256, 512] [128, 512, 14]
